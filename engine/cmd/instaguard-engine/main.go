// Package main is the entry point for the InstaGuard firewall engine.
package main
...
import (
	...
	"github.com/raushan-nexapp/instaguard/internal/api"
...
func rootHandler(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintln(w, "🛡️  InstaGuard Engine v"+Version)
	fmt.Fprintln(w, "")
...
func main() {
	dbPath := os.Getenv("INSTAGUARD_DB")
	if dbPath == "" {
		dbPath = "data/instaguard.db"
	}
	...
	addr := ":8080"
	log.Println("╔════════════════════════════════════════╗")
	log.Println("║  🛡️  InstaGuard Engine v" + Version + "           ║")
	log.Println("╚════════════════════════════════════════╝")
